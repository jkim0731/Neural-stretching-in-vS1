import design_matrix_touch_combined as dm_touch_combined
import pandas as pd
...
def make_and_save_design_matrix_touch_combined(mouse, plane, session, base_dir):
    print(f'processing {mouse:03} plane {plane} session {session:03}')
    design_df, _ = dm_touch_combined.make_design_dataframe(mouse, plane, session, base_dir)
    save_dir = base_dir / f'{mouse:03}/plane_{plane}/{session:03}/plane0/roi/glm/touch_combined'
    save_dir.mkdir(exist_ok=True, parents=True)
    design_df.to_pickle(save_dir / 'design_touch_combined.pkl')
...
if __name__ == '__main__':
    ...
    with Pool(processes=18) as pool:
        pool.starmap(make_and_save_design_matrix_touch_combined, [(mouse, plane, int(session), base_dir)
    ...
    # took 6 min for all expert use mice
