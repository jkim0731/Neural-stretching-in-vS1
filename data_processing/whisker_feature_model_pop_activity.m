% Reconstructing neuronal activities by whisker feature models, and then
% running population activity analysis (PCA)
% 
% First, reconstruct the whole session, save as the same format as in spk,
% and then run the rest as in spk.
% (d200826_neural_stretching_confirm_and_control.m)
% Save 14 models (full model, whisker-only model, each loo model) for each
% session in one file (whisker_model_touch_neurons_JK%03dS%02d).
% 
% Then, test with whisker features only (removing other behavioral
% categories such as sound cue, whisking, licking, and drinking)
% 
% Finally, repeat with removing each whisker features from the 
% whisker-only model
% 
% 2020/08/31 JK
...
for mi = 1 : length(mice)
    for si = 1 : length(sessions{mi})
        ...
        savefn = sprintf('whisker_model_touch_neurons_JK%03dS%02d', mouse, session);
        ...
        whiskerLooModels = cell(length(cIDAll),12);
        deFull = zeros(length(cIDAll),1);
        deWhiskerOnly = zeros(length(cIDAll),1);
        deWhiskerFeatures = zeros(length(cIDAll),12);
        parfor ci = 1 : length(cIDAll)
            ...
            for fi = 1 : 12
                tempInputFeature = tempInput;
                tempInputFeature(:,2+(fi-1)*3:1+fi*3) = 0;
                whiskerLooModels{ci,fi} = exp(tempInputFeature*meanCoeffsWKV(ci,:)');
                featureOutLogLikelihood = nansum(log(poisspdf(spkTest', whiskerLooModels{ci,fi})));
                deWhiskerFeatures(ci,fi) = (featureOutLogLikelihood - nullLogLikelihood)/(saturatedLogLikelihood - nullLogLikelihood);
            end
        end
        ...
        atBehaviorAll = cell(length(cIDAll), 1);
        numTouchAngleAll = cell(length(cIDAll), 1);
        parfor ci = 1 : length(cIDAll)
...
%             fprintf('cell # %d\n', ci)
            ...
            behaviorAngle = cell(length(indPartial), length(angles));
            ...
            for ai = 1 : length(angles)
                ...
                % behavior removal
                for ii = 1 : length(indPartial)
                    behaviorAngle{ii, ai} = cell(length(tempTrialInd),1);
                end
                ...
                for ti = 1 : length(tempTrialInd)
                    ...
                    for ii = 1 : length(indPartial)
                        tempCoeff = meanCoeffsWKV(ci,:);
                        tempCoeff(1,indPartial{ii}+1) = 0;
                        behaviorAngle{ii,ai}{ti} = exp(tempInput * tempCoeff');
                    end
            ...
            touchResponseBehavior = cell(length(indPartial), length(angles));
            for ai = 1 : length(angles)
                ...
                for ii = 1 : length(indPartial)
                    touchResponseBehavior{ii,ai} = zeros(length(spikeAngle{ai}), 1);
                end
                for ti = 1 : length(spikeAngle{ai})
                    ...
                    for ii = 1 : length(indPartial)
                        touchResponseBehavior{ii,ai}(ti) = sum( behaviorAngle{ii,ai}{ti}(touchFramesAngle{ai}{ti}) - nanmean( behaviorAngle{ii,ai}{ti}(baselineFramesAngle{ai}{ti}) )  ) / numTouchAngle{ai}{ti};
                    end
                end
            end
            ...
            atBehavior = cellfun(@mean, touchResponseBehavior);
            ...
            tempCorrBehavior = zeros(1,length(indPartial));
            for ii = 1 : length(indPartial)
                tempCorrBehavior(1,ii) = corr(atSpike', atBehavior(ii,:)');
            end
            
            atSpikeAll{ci} = atSpike;
            atFullAll{ci} = atFull;
            atWhiskerAll{ci} = atWhisker;
            atBehaviorAll{ci} = atBehavior;
            
            atCorrFull{ci} = tempCorrFull;
            atCorrWhisker{ci} = tempCorrWhisker;
            atCorrBehavior{ci} = tempCorrBehavior;
            
            spikeAngleAll{ci} = touchResponseSpike;
            fullModelAngleAll{ci} = touchResponseFull;
            whiskerOnlyAngleAll{ci} = touchResponseWhisker;
            
            numTouchAngleAll{ci}= numTouchAngle;
        end
        
        save(sprintf('%s%03d\\%s', baseDir, mouse, savefn), '*All', 'atCorr*', 'beforePoleUpFrames', 'touchFrames', 'numTouchPreAnswer', 'deFull', 'deWhiskerOnly', 'deWhiskerFeatures', 'numTouchAngleAll')
...
tune = load([baseDir, 'angle_tuning_summary_preAnswer_perTouch_NC_PTC']);
% naive first
for mi = 1 : length(mice)
    mouse = mice(mi);
