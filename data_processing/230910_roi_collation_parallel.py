...
def process_mask_merging(mouse, plane, session):
    plane_dir = BASE_DIR / f'{mouse:03}' / f'plane_{plane}' / f'{session}' / 'plane0'
    roi_dir = plane_dir / 'roi'
    if not roi_dir.exists():
        roi_dir.mkdir()
    final_fn = f'final_roi_results_{mouse:03d}_plane_{plane}_{session}_wo_dendrite_filtering.npy'
    if (roi_dir / final_fn).exists():
        print(f'{mouse:03d}_plane_{plane}_{session} already exists.')
        return
    
    t0 = time()
    print(f'Processing {mouse:03d}_plane_{plane}_{session}...')

    merged_fn = f'merged_masks_{mouse:03d}_plane_{plane}_{session}.npy'
    if (roi_dir / merged_fn).exists():
        print('Merged mask already exists. Loading...')
        results_merged_masks = np.load(roi_dir / merged_fn, allow_pickle=True).item()

        merged_masks = [results_merged_masks['meanImg'],
                        results_merged_masks['meanImgE'],
                        results_merged_masks['max_proj']]
    else:
        ...
        np.save(roi_dir / merged_fn, results_merged_masks)
    ...
    np.save(roi_dir / final_fn, final_roi_results)
    t1 = time()
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(num_cores) as pool:
        pool.starmap(process_mask_merging, session_tuples, chunksize=1)
